...
class FileProcessor:
        ...
        try:
            ...
            for status_col in status_cols:
                if status_col in output_df.columns:
                    mask = output_df[status_col].astype(str).str.strip().str.lower() == 'in progress today'
                    conv_count = mask.sum()
                    if conv_count > 0:
    ...
    def process_chat_agent_final_step(self, df, output_file, selected_handlers, chat_agent_info, prev_file=None):
        """FINAL STEP: Process Chat Agent cases after all other processing is complete
        
        This is the LAST step in the process:
        1. After all handlers are assigned
        2. After SMS/Email replies are processed
        3. After everything is settled
        
        Logic:
        - Count total in_progress cases across ALL handler sheets
        - Divide equally among all active selected handlers
        - Pull the BOTTOM cases (newest) from each handler's in_progress queue
        - Create/update Chat Agent's Cases sheet with:
          * Preserved previous cases
          * New in_progress cases with current Chat Agent name
        - Exclude Chat Agent from final action and company counters
        
        Args:
            df: Current output dataframe with all assignments
            output_file: Path where output will be written
            selected_handlers: List of active handler names (NOT including Chat Agent)
            chat_agent_info: Chat Agent configuration dict
            prev_file: Path to previous output file for preservation
            
        Returns:
            tuple: (updated_df, chat_agent_sheet_df)
        """
        try:
            if not chat_agent_info or not chat_agent_info.get('enabled'):
                self.logger.info("Chat Agent not enabled - skipping final step")
                return df, pd.DataFrame()
            
            chat_agent_supporter_name = chat_agent_info.get('supporter_name', 'Chat Agent')
            self.logger.info(f"\n{'='*70}")
            self.logger.info(f"CHAT AGENT FINAL STEP: Processing for: {chat_agent_supporter_name}")
            self.logger.info(f"{'='*70}")
            
            # Get active handlers (exclude Chat Agent)
            ...
            if 'Assigned To' in df.columns and not merged_chat_agent_df.empty:
                if case_col and case_col in merged_chat_agent_df.columns and case_col in df.columns:
                    ...
                    if removed_count > 0:
                        # DO NOT delete them from df! We want them in the PA Cases sheet!
                        # The individual handler sheet generation naturally excludes them based on Assigned To = Chat Agent.
                        df = df.drop(columns=['_temp_cn'], errors='ignore')
                        self.logger.info(f"  Kept {removed_count} Chat Agent cases in main df (assigned to {chat_agent_supporter_name}) so they appear in PA Cases")
                    else:
                        df = df.drop(columns=['_temp_cn'], errors='ignore')
            ...
            self.logger.error(f"Error in process_chat_agent_final_step: {str(e)}")
            import traceback
            self.logger.error(f"Traceback: {traceback.format_exc()}")
            return df, pd.DataFrame()
...
class FinalProcessor:
    ...
    def process_final_output(self, output_df, output_file, processing_stats=None, sms_file=None, email_file=None, prev_file=None, duplicate_company_cases=None, selected_handlers=None, chat_agent_info=None, chat_agent_sheet_df=None, protect_companies=False):
        try:
            ...
            with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
                ...
                if chat_agent_info and chat_agent_info.get('enabled'):
                    ...
                    # Always create the Chat Agent's Cases sheet, even if empty
                    if chat_agent_sheet_df is not None and not chat_agent_sheet_df.empty:
                        # Write the prepared Chat Agent cases
                        chat_agent_sheet_df.to_excel(writer, sheet_name=sheet_name, index=False)
                        self.auto_adjust_columns(writer, chat_agent_sheet_df, sheet_name)
                        
                        # Log the content for clarity
                        if 'Assigned To' in chat_agent_sheet_df.columns:
                            assigned_to_counts = chat_agent_sheet_df['Assigned To'].value_counts()
                            self.logger.info(f"✓ Created '{sheet_name}' with {len(chat_agent_sheet_df)} cases")
                            self.logger.info(f"  'Assigned To' distribution: {assigned_to_counts.to_dict()}")
                        else:
                            self.logger.info(f"✓ Created '{sheet_name}' with {len(chat_agent_sheet_df)} cases")
                    else:
                        # Create empty Chat Agent sheet with proper column headers from output_df
                        self.logger.info(f"Chat Agent has no new cases to assign - creating empty sheet with headers")
                        empty_chat_agent_df = output_df.iloc[0:0].copy()  # Get column structure but no rows
                        empty_chat_agent_df.to_excel(writer, sheet_name=sheet_name, index=False)
                    ...
                    # ================================================================
                    # Chat Agent NOT selected for today — preserve previous sheet as-is
                    # ================================================================
                    CHAT_AGENT_SHEET_NAME = "Chat Agent's Cases"
                    preserved_chat_df = None
                    if prev_file and os.path.exists(prev_file):
                        try:
                            with pd.ExcelFile(prev_file) as prev_excel:
                                if CHAT_AGENT_SHEET_NAME in prev_excel.sheet_names:
                                    preserved_chat_df = pd.read_excel(prev_file, sheet_name=CHAT_AGENT_SHEET_NAME)
                                    self.logger.info(
                                        f"Chat Agent not selected today — preserving previous "
                                        f"'{CHAT_AGENT_SHEET_NAME}' sheet with {len(preserved_chat_df)} rows"
                                    )
                        except Exception as e:
                            self.logger.warning(f"Could not load previous Chat Agent sheet for preservation: {e}")

                    if preserved_chat_df is not None and not preserved_chat_df.empty:
                        preserved_chat_df.to_excel(writer, sheet_name=CHAT_AGENT_SHEET_NAME, index=False)
                        self.auto_adjust_columns(writer, preserved_chat_df, CHAT_AGENT_SHEET_NAME)
                        self.logger.info(f"✓ Preserved '{CHAT_AGENT_SHEET_NAME}' from previous file unchanged")
                    else:
                        self.logger.info(
                            f"Chat Agent not selected and no previous '{CHAT_AGENT_SHEET_NAME}' sheet found — skipping"
                        )
                ...
                if companies_df is not None and not companies_df.empty:
                    ...
                    # Conditionally protect the sheet based on user choice
                    if protect_companies:
                        self.protect_worksheet(writer, 'Companies', password='artadmin')
                        self.logger.info("✓ Companies sheet password-protected")
                    else:
                        self.logger.info("ℹ Companies sheet protection skipped (unchecked by user)")
    ...
    def create_companies_sheet_with_preservation(self, new_duplicates, prev_file, current_df, selected_handlers=None, chat_agent_info=None):
        """
    ...
    - [x] Implement Locking Logic (Password: 'artadmin', View-only) in `final_processor.py` <!-- id: 15 -->Email groups
        ...
        Args:
            ...
            chat_agent_info: Chat Agent configuration (to exclude from companies assignment)
        ...
        try:
            ...
            # Step 4: Assign cases to handlers (Round Robin by Email) - EXCLUDE CHAT AGENT
            companies_df = self._assign_companies_cases(companies_df, selected_handlers, chat_agent_info=chat_agent_info)
    ...
    def create_counters_sheet(self, writer, df, prev_df=None, prev_file=None, chat_agent_info=None, selected_handlers=None):
        try:
            ...
            # ====== RULE: Counter sheet EXCLUDES ALL Chat Agent cases ======
            # Load previous Chat Agent cases to exclude from counters
            chat_agent_case_numbers = set()
            CHAT_AGENT_SHEET_NAME = "Chat Agent's Cases"
            ...
            if prev_file and os.path.exists(prev_file):
                try:
                    ...
                    if CHAT_AGENT_SHEET_NAME in excel_file.sheet_names:
                        ...
                        case_col = None
                        for col in prev_chat_agent_df.columns:
                            if 'case' in col.lower() and 'number' in col.lower():
                                case_col = col
                                break
                        
                        if case_col:
                            chat_agent_case_numbers = set(prev_chat_agent_df[case_col].astype(str).unique())
                            self.logger.info(f"✓ Loaded {len(chat_agent_case_numbers)} Chat Agent case numbers from previous file for exclusion from counters")
                except Exception as e:
                    self.logger.warning(f"Could not load previous Chat Agent cases for exclusion: {str(e)}")
            
            # Filter out Chat Agent cases from counter calculations
            case_col = None
            for col in df.columns:
                if 'case' in col.lower() and 'number' in col.lower():
                    case_col = col
            ...
            counter_df = df.copy()
            if case_col and chat_agent_case_numbers:
                # Exclude Chat Agent cases from counter dataframe
                counter_df = counter_df[~counter_df[case_col].astype(str).isin(chat_agent_case_numbers)].copy()
                excluded_count = len(df) - len(counter_df)
                self.logger.info(f"RULE: Excluding {excluded_count} Chat Agent cases from counter calculations")
            ...
            self.logger.info(f"Counter will show ALL handlers (both selected and unselected, Chat Agent cases excluded)")
            
            # Get Chat Agent name if enabled
            chat_agent_name = None
            if chat_agent_info and chat_agent_info.get('enabled'):
                chat_agent_name = chat_agent_info.get('supporter_name', 'Chat Agent')
                self.logger.info(f"Chat Agent '{chat_agent_name}' is enabled - ALL its cases excluded from counters")
            
            # ====== GET ALL HANDLERS FROM DATA (selected + unselected) ======
            # Include main handlers and any other handlers in the data
            MAIN_HANDLERS = ["Adam", "Ehab", "Teama", "Ibrahim", "Moamen"]
            
            # Get handlers from counter dataframe (without Chat Agent cases)
            handlers_from_data = counter_df['Assigned To'].dropna().astype(str).str.strip().unique()
            ...
            # Remove Chat Agent from handler list (its cases are excluded from counters)
            ...
            self.logger.info(f"(Chat Agent cases excluded - will NOT appear in any counter row)")
            ...
            for handler_row_idx, handler in enumerate(handler_list, start_handler_row):
                ...
                for col_idx, (col_name, _) in enumerate(progress_columns, 1):
                    if col_name == "Total":
                        worksheet.write_formula(row_idx, col_idx, f"=SUM(B{row_idx+1}:F{row_idx+1})")
            ...
            if chat_agent_info and chat_agent_info.get('enabled'):
                ...
                # Total: =SUM(B:F for this row)
                worksheet.write_formula(row_idx, 6, f"=SUM(B{row_idx+1}:F{row_idx+1})")
    ...
    def _assign_companies_cases(self, df, selected_handlers, chat_agent_info=None):
        """
        ...
        CHAT AGENT EXCLUDED: Companies cases are NOT assigned to Chat Agent,
        even if Chat Agent is enabled. Companies handling is for regular handlers only.
        """
        try:
            ...
            # EXCLUDE Chat Agent from selected_handlers
            handlers_for_assignment = selected_handlers
            if chat_agent_info and chat_agent_info.get('enabled'):
                chat_agent_name = chat_agent_info.get('supporter_name', 'Chat Agent')
                handlers_for_assignment = [h for h in selected_handlers if h != chat_agent_name]
                self.logger.info(f"Excluding Chat Agent '{chat_agent_name}' from companies assignment")
            
            if not handlers_for_assignment:
                self.logger.warning("No handlers available for companies assignment after excluding Chat Agent")
                return df
            
            self.logger.info(f"Processing {len(df)} cases for assignment to {len(handlers_for_assignment)} handlers...")
            ...
            for email in unique_emails:
                if email not in email_handler_map:
                    # Get next handler
                    handler = handlers_for_assignment[handler_idx % len(handlers_for_assignment)]
            ...
            for handler in handlers_for_assignment:
