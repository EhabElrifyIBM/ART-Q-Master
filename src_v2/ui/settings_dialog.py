# ============================================================================
# settings_dialog.py - Settings/Preferences Dialog
# ============================================================================
# Phase 5.4: UI Settings & Customization
#
# Provides user-facing controls for:
# - Theme switching (Light/Dark mode)
# - Font size adjustment (80% to 200%)
# - Keyboard navigation and screen reader options
# - Sound effects on/off
# - Auto-save preferences
# ============================================================================

import sys
import os
from PyQt5.QtWidgets import (
    QDialog, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QSlider,
    QComboBox, QCheckBox, QGroupBox, QWidget, QGridLayout, QSpinBox,
    QMessageBox, QFrame, QApplication, QRadioButton, QButtonGroup
)
from PyQt5.QtCore import Qt, pyqtSignal
from PyQt5.QtGui import QFont

# Typography system imports
from ui.typography import TypographySystem, FontSizePreset

# Lazy imports to avoid QApplication issues
theme_manager = None
accessibility_manager = None
app = None


class SettingsDialog(QDialog):
    """
    Settings dialog for user preferences.
    
    Signals:
    - theme_changed(light/dark)
    - font_size_changed(scale: 0.8-2.0)
    - accessibility_changed(settings_dict)
    """
    
    theme_changed = pyqtSignal(str)  # 'light' or 'dark'
    font_size_changed = pyqtSignal(float)  # 0.8 to 2.0
    accessibility_changed = pyqtSignal(dict)  # Keyboard, screen reader, etc
    
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Settings & Preferences")
        self.setMinimumWidth(600)
        self.setMinimumHeight(650)
        self.setModal(True)
        
        # Initialize managers
        self._init_managers()
        
        # Get current settings
        self.current_theme = getattr(theme_manager, 'current_theme', None)
        if self.current_theme:
            self.current_theme = self.current_theme.value if hasattr(self.current_theme, 'value') else 'light'
        else:
            self.current_theme = 'light'
        
        # Get current font preset (default to 'normal')
        self.current_preset = "normal"
        self.current_scale = getattr(accessibility_manager, 'text_scaler', None)
        if self.current_scale:
            scale_value = getattr(self.current_scale, 'scale_factor', 22)
            # If scale_value is a percentage (e.g., 1.0, 1.5), convert to pixels
            # If it's already in pixels (15-30), use as-is
            if scale_value < 15:
                # It's a scale factor, convert to pixels (assume base 15px)
                self.current_scale = max(15, min(30, int(scale_value * 15)))
            else:
                # It's already in pixels
                self.current_scale = max(15, min(30, int(scale_value)))
        else:
            self.current_scale = 16  # Default to 16px (100% = normal)
        
        # Setup UI
        self._setup_ui()
        
        # Load current preset after UI is setup
        self._load_current_preset()
        
        # Apply theme to dialog
        if theme_manager:
            self._apply_theme()
    
    def _init_managers(self):
        """Initialize theme and accessibility managers."""
        global theme_manager, accessibility_manager, app
        
        try:
            from ui.theme_manager import get_theme_manager
            from ui.accessibility_helper import get_accessibility_manager
            
            theme_manager = get_theme_manager()
            accessibility_manager = get_accessibility_manager()
            app = QApplication.instance()
        except Exception as e:
            print(f"[WARNING] Could not load managers: {e}")
    
    def _setup_ui(self):
        """Setup dialog UI components."""
        main_layout = QVBoxLayout(self)
        main_layout.setContentsMargins(25, 25, 25, 25)
        main_layout.setSpacing(20)
        
        # Title
        title = QLabel("Settings & Preferences")
        title_font = QFont()
        title_font.setPointSize(17)
        title_font.setBold(True)
        title.setFont(title_font)
        main_layout.addWidget(title)
        
        # Separator
        separator = QFrame()
        separator.setFrameShape(QFrame.HLine)
        separator.setFrameShadow(QFrame.Sunken)
        main_layout.addWidget(separator)
        
        # ===== APPEARANCE SECTION =====
        appearance_group = QGroupBox("Appearance")
        appearance_layout = QVBoxLayout()
        appearance_layout.setSpacing(12)
        
        # Theme selector
        theme_layout = QHBoxLayout()
        theme_label = QLabel("Theme:")
        theme_font = QFont()
        theme_font.setPointSize(15)
        theme_label.setFont(theme_font)
        theme_layout.addWidget(theme_label)
        
        self.theme_combo = QComboBox()
        self.theme_combo.addItems(["☀️ Light", "🌙 Dark"])
        self.theme_combo.setCurrentIndex(0 if self.current_theme == 'light' else 1)
        self.theme_combo.currentIndexChanged.connect(self._on_theme_changed)
        self.theme_combo.setMinimumWidth(200)
        theme_font.setPointSize(13)
        self.theme_combo.setFont(theme_font)
        theme_layout.addWidget(self.theme_combo)
        theme_layout.addStretch()
        appearance_layout.addLayout(theme_layout)
        
        appearance_group.setLayout(appearance_layout)
        main_layout.addWidget(appearance_group)
        
        # ===== FONT & TEXT SECTION =====
        font_group = QGroupBox("Font & Text")
        font_layout = QVBoxLayout()
        font_layout.setSpacing(12)
        
        # Font Size Presets Section
        font_size_label = QLabel("Font Size:")
        font_size_label.setStyleSheet("font-weight: bold; font-size: 14px;")
        font_layout.addWidget(font_size_label)
        
        # Radio button group for presets
        self.preset_button_group = QButtonGroup(self)
        presets = [
            ("Small (87.5%)", "small", "More content fits on screen"),
            ("Normal (100%) ⭐", "normal", "Recommended for most users"),
            ("Large (112.5%)", "large", "Better readability"),
            ("XLarge (125%)", "xlarge", "Maximum accessibility")
        ]
        
        for label, value, description in presets:
            radio = QRadioButton(label)
            radio.setProperty("preset_value", value)
            radio.setToolTip(description)
            radio.setStyleSheet("font-size: 13px; padding: 5px;")
            
            # Set default to normal
            if value == "normal":
                radio.setChecked(True)
            
            self.preset_button_group.addButton(radio)
            radio.toggled.connect(self._on_preset_changed)
            font_layout.addWidget(radio)
        
        font_layout.addSpacing(10)
        font_group.setLayout(font_layout)
        main_layout.addWidget(font_group)
        
        # ===== ACCESSIBILITY SECTION =====
        accessibility_group = QGroupBox("Accessibility (WCAG 2.1 AA)")
        accessibility_layout = QVBoxLayout()
        accessibility_layout.setSpacing(12)
        
        # High contrast mode
        self.high_contrast_check = QCheckBox("Enable High Contrast Mode")
        self.high_contrast_check.setToolTip("Increases contrast for better visibility")
        self.high_contrast_check.setStyleSheet("font-size: 13px; padding: 5px;")
        self.high_contrast_check.stateChanged.connect(self._on_high_contrast_changed)
        accessibility_layout.addWidget(self.high_contrast_check)
        
        # Focus indicator size
        focus_layout = QHBoxLayout()
        focus_label = QLabel("Focus Indicator Size:")
        focus_label.setStyleSheet("font-size: 13px;")
        focus_layout.addWidget(focus_label)
        
        self.focus_size_combo = QComboBox()
        self.focus_size_combo.addItems(["2px (Subtle)", "3px (Standard - WCAG)", "4px (Enhanced)"])
        self.focus_size_combo.setCurrentIndex(1)  # Default to 3px
        self.focus_size_combo.setStyleSheet("font-size: 13px;")
        self.focus_size_combo.currentIndexChanged.connect(self._on_focus_size_changed)
        focus_layout.addWidget(self.focus_size_combo)
        focus_layout.addStretch()
        accessibility_layout.addLayout(focus_layout)
        
        # Keyboard navigation hints
        self.keyboard_hints_check = QCheckBox("Show Keyboard Navigation Hints")
        self.keyboard_hints_check.setChecked(True)
        self.keyboard_hints_check.setToolTip("Display keyboard shortcuts in tooltips")
        self.keyboard_hints_check.setStyleSheet("font-size: 13px; padding: 5px;")
        accessibility_layout.addWidget(self.keyboard_hints_check)
        
        # Screen reader mode
        self.screen_reader_check = QCheckBox("Optimize for Screen Readers")
        self.screen_reader_check.setToolTip("Enhanced ARIA labels and announcements")
        self.screen_reader_check.setStyleSheet("font-size: 13px; padding: 5px;")
        self.screen_reader_check.stateChanged.connect(self._on_screen_reader_changed)
        accessibility_layout.addWidget(self.screen_reader_check)
        
        # Touch target enforcement
        self.touch_targets_check = QCheckBox("Enforce Minimum Touch Targets (44x44px)")
        self.touch_targets_check.setChecked(True)
        self.touch_targets_check.setToolTip("Ensures all buttons meet WCAG touch target size")
        self.touch_targets_check.setStyleSheet("font-size: 13px; padding: 5px;")
        self.touch_targets_check.stateChanged.connect(self._on_touch_targets_changed)
        accessibility_layout.addWidget(self.touch_targets_check)
        
        # Info label
        info_label = QLabel("ℹ️ These settings help meet WCAG 2.1 Level AA compliance")
        info_label.setStyleSheet("font-size: 11px; color: #525252; padding: 5px;")
        info_label.setWordWrap(True)
        accessibility_layout.addWidget(info_label)
        
        accessibility_group.setLayout(accessibility_layout)
        main_layout.addWidget(accessibility_group)
        
        # ===== BUTTONS SECTION =====
        button_layout = QHBoxLayout()
        button_layout.setSpacing(12)
        
        reset_btn = QPushButton("↺ Reset to Defaults")
        reset_btn.setMinimumHeight(40)
        reset_btn.clicked.connect(self._on_reset_defaults)
        reset_font = QFont()
        reset_font.setPointSize(13)
        reset_btn.setFont(reset_font)
        reset_btn.setStyleSheet("""
            QPushButton {
                background-color: #E0E0E0;
                color: #333;
                border: none;
                border-radius: 4px;
                padding: 8px 16px;
            }
            QPushButton:hover {
                background-color: #D0D0D0;
            }
        """)
        button_layout.addWidget(reset_btn)
        
        button_layout.addStretch()
        
        close_btn = QPushButton("✓ Close Settings")
        close_btn.setMinimumHeight(40)
        close_btn.setMinimumWidth(150)
        close_btn.clicked.connect(self.accept)
        close_btn.setFont(reset_font)
        close_btn.setStyleSheet("""
            QPushButton {
                background-color: #0f62fe;
                color: #ffffff;
                border: none;
                border-radius: 4px;
                padding: 8px 16px;
                font-weight: bold;
            }
            QPushButton:hover {
                background-color: #0353e9;
            }
        """)
        button_layout.addWidget(close_btn)
        
        main_layout.addStretch()
        main_layout.addLayout(button_layout)
    
    def _on_theme_changed(self, index):
        """Handle theme selection change."""
        new_theme = 'light' if index == 0 else 'dark'
        
        if theme_manager and app:
            try:
                from ui.theme_manager import ThemeMode
                from ui.services import get_v2_settings_bus
                
                # Set theme in manager
                theme_mode = ThemeMode.LIGHT if new_theme == 'light' else ThemeMode.DARK
                theme_manager.set_theme(theme_mode)
                
                # Apply stylesheet to entire application
                stylesheet = theme_manager.get_stylesheet()
                app.setStyleSheet(stylesheet)
                
                # Broadcast change to all observers using V2SettingsBus SINGLETON
                settings_bus = get_v2_settings_bus()
                settings_bus.set_theme(new_theme)  # Use set_theme() which emits the signal
                
                self.current_theme = new_theme
                self._apply_theme()
                self.theme_changed.emit(new_theme)
                print(f"[INFO] Theme changed to {new_theme} and applied to application")
            except Exception as e:
                print(f"[ERROR] Failed to change theme: {e}")
    
    def _on_preset_changed(self, checked):
        """Handle font preset change"""
        if not checked:
            return
        
        sender = self.sender()
        if sender:
            preset_value = sender.property("preset_value")
            if preset_value:
                try:
                    from ui.services import get_v2_settings_bus
                    
                    # Emit to V2SettingsBus
                    settings_bus = get_v2_settings_bus()
                    settings_bus.font_preset_changed.emit(preset_value)
                    
                    # Convert to point size with better scaling
                    # Small: 14pt (actually small), Normal: 18pt, Large: 22pt, XLarge: 26pt
                    preset_enum = FontSizePreset.from_string(preset_value)
                    point_size = int(16 * preset_enum.get_multiplier())
                    settings_bus.font_size_changed.emit(point_size)
                    
                    # Apply font scaling to the entire application (but not this dialog)
                    self._apply_font_scale_to_app(point_size)
                    
                    # Restore this dialog's fixed font sizes
                    self._restore_dialog_fonts()
                    
                    # Update accessibility manager if available
                    if accessibility_manager:
                        accessibility_manager.text_scaler.scale_factor = point_size
                    
                    self.current_scale = point_size
                    self.font_size_changed.emit(point_size)
                    
                    # SAVE TO PERSISTENT STORAGE
                    self._save_preset_to_config(preset_value)
                    
                    print(f"[INFO] Font preset changed to {preset_value} ({point_size}pt) and applied to application")
                except Exception as e:
                    print(f"[ERROR] Failed to change font preset: {e}")
    
    def _apply_font_scale_to_app(self, point_size):
        """Apply font size to entire application (point_size: 15-30pt)."""
        try:
            if not app:
                return
            
            # Clamp to valid range
            point_size = max(15, min(30, point_size))
            
            base_font = app.font()
            base_font.setPointSize(int(point_size))
            
            # Apply to application
            app.setFont(base_font)
            print(f"[INFO] Applied font size {point_size}pt to application")
        except Exception as e:
            print(f"[WARNING] Could not apply font size: {e}")
    
    def _restore_dialog_fonts(self):
        """Restore fixed font sizes for this dialog (prevent it from being affected by app font changes)."""
        try:
            # Create a fixed font for the dialog
            fixed_font = QFont()
            fixed_font.setPointSize(10)  # Small, fixed size for dialog
            
            # Apply to all widgets in this dialog
            for widget in self.findChildren(QWidget):
                if hasattr(widget, 'setFont'):
                    widget.setFont(fixed_font)
            
            # Set specific sizes for key elements
            # Main title
            for label in self.findChildren(QLabel):
                if 'Settings' in label.text() and 'Preferences' in label.text():
                    title_font = QFont()
                    title_font.setPointSize(14)
                    title_font.setBold(True)
                    label.setFont(title_font)
                    break
            
            # Group box titles (Appearance, Font & Text)
            for groupbox in self.findChildren(QGroupBox):
                group_font = QFont()
                group_font.setPointSize(11)
                group_font.setBold(True)
                groupbox.setFont(group_font)
            
            print("[DEBUG] Dialog fonts restored to fixed sizes")
        except Exception as e:
            print(f"[WARNING] Could not restore dialog fonts: {e}")
    
    def _save_preset_to_config(self, preset: str):
        """Save font preset to config file"""
        try:
            import json
            from pathlib import Path
            
            # Get config file path
            config_path = Path(__file__).parent.parent / "config.json"
            
            # Read current config
            if config_path.exists():
                with open(config_path, 'r') as f:
                    config = json.load(f)
            else:
                config = {}
            
            # Add/update font_preset
            if 'ui_settings' not in config:
                config['ui_settings'] = {}
            config['ui_settings']['font_preset'] = preset
            
            # Save config
            with open(config_path, 'w') as f:
                json.dump(config, f, indent=4)
                
            print(f"[INFO] Saved font preset: {preset}")
        except Exception as e:
            print(f"[WARNING] Could not save font preset: {e}")
    
    def _load_current_preset(self):
        """Load and set the current font preset from settings."""
        try:
            # Try to load from config.json first
            import json
            from pathlib import Path
            
            config_path = Path(__file__).parent.parent / "config.json"
            if config_path.exists():
                with open(config_path, 'r') as f:
                    config = json.load(f)
                
                saved_preset = config.get('ui_settings', {}).get('font_preset', 'normal')
                current_preset = saved_preset
                print(f"[INFO] Loaded font preset from config: {current_preset}")
            else:
                # Fallback to normal
                current_preset = 'normal'
                print(f"[INFO] No config found, using default preset: {current_preset}")
        except Exception as e:
            print(f"[WARNING] Could not load preset from config: {e}")
            # Fallback: determine preset from current point size
            # Small: 14pt, Normal: 18pt, Large: 22pt, XLarge: 26pt
            scale = self.current_scale if self.current_scale else 18
            if scale <= 16:
                current_preset = "small"
            elif scale <= 20:
                current_preset = "normal"
            elif scale <= 24:
                current_preset = "large"
            else:
                current_preset = "xlarge"
        
        # Set the correct radio button
        for button in self.preset_button_group.buttons():
            if button.property("preset_value") == current_preset:
                button.setChecked(True)
                break
        
        self.current_preset = current_preset
        print(f"[INFO] Set current font preset: {current_preset}")
    
    def _on_reset_defaults(self):
        """Reset all settings to defaults."""
        reply = QMessageBox.question(
            self,
            "Reset Settings",
            "Reset all settings to defaults?\n\nTheme: Light\nFont Size: Normal (100%)",
            QMessageBox.Yes | QMessageBox.No
        )
        
        if reply == QMessageBox.Yes:
            # Reset theme
            self.theme_combo.setCurrentIndex(0)
            
            # Reset font size to normal preset
            for button in self.preset_button_group.buttons():
                if button.property("preset_value") == "normal":
                    button.setChecked(True)
                    break
            
            print("[INFO] Settings reset to defaults")
    
    def _on_high_contrast_changed(self, state):
        """Handle high contrast mode toggle."""
        try:
            if accessibility_manager:
                if state == Qt.Checked:
                    accessibility_manager.enable_high_contrast()
                    print("[INFO] High contrast mode enabled")
                else:
                    accessibility_manager.disable_high_contrast()
                    print("[INFO] High contrast mode disabled")
                
                # Emit accessibility changed signal
                self.accessibility_changed.emit(accessibility_manager.get_accessibility_info())
        except Exception as e:
            print(f"[ERROR] Failed to toggle high contrast: {e}")
    
    def _on_focus_size_changed(self, index):
        """Handle focus indicator size change."""
        try:
            # Map index to pixel size: 0=2px, 1=3px, 2=4px
            sizes = [2, 3, 4]
            size = sizes[index]
            
            if accessibility_manager:
                accessibility_manager.focus_indicator_size = size
                print(f"[INFO] Focus indicator size set to {size}px")
                
                # Emit accessibility changed signal
                self.accessibility_changed.emit(accessibility_manager.get_accessibility_info())
        except Exception as e:
            print(f"[ERROR] Failed to change focus size: {e}")
    
    def _on_screen_reader_changed(self, state):
        """Handle screen reader mode toggle."""
        try:
            if accessibility_manager:
                accessibility_manager.screen_reader_mode = (state == Qt.Checked)
                mode_str = "enabled" if state == Qt.Checked else "disabled"
                print(f"[INFO] Screen reader mode {mode_str}")
                
                # Emit accessibility changed signal
                self.accessibility_changed.emit(accessibility_manager.get_accessibility_info())
        except Exception as e:
            print(f"[ERROR] Failed to toggle screen reader mode: {e}")
    
    def _on_touch_targets_changed(self, state):
        """Handle touch target enforcement toggle."""
        try:
            if accessibility_manager:
                accessibility_manager.enforce_touch_targets = (state == Qt.Checked)
                mode_str = "enabled" if state == Qt.Checked else "disabled"
                print(f"[INFO] Touch target enforcement {mode_str}")
                
                # Emit accessibility changed signal
                self.accessibility_changed.emit(accessibility_manager.get_accessibility_info())
        except Exception as e:
            print(f"[ERROR] Failed to toggle touch targets: {e}")
    
    def _apply_theme(self):
        """Apply current theme to dialog."""
        if not theme_manager:
            return
        
        try:
            # Get theme colors
            bg_color = "#FAFAFA" if self.current_theme == 'light' else "#1E1E1E"
            text_color = "#333333" if self.current_theme == 'light' else "#FFFFFF"
            
            self.setStyleSheet(f"""
                QDialog {{
                    background-color: {bg_color};
                }}
                QLabel {{
                    color: {text_color};
                }}
                QGroupBox {{
                    color: {text_color};
                    border: 1px solid #CCCCCC;
                    border-radius: 4px;
                    padding-top: 8px;
                    margin-top: 8px;
                }}
                QGroupBox::title {{
                    subcontrol-origin: margin;
                    left: 10px;
                    padding: 0 3px 0 3px;
                }}
                QCheckBox {{
                    color: {text_color};
                }}
                QComboBox {{
                    color: {text_color};
                    background-color: {"#FFFFFF" if self.current_theme == 'light' else "#2E2E2E"};
                    border: 1px solid #999;
                    border-radius: 4px;
                    padding: 6px;
                }}
            """)
        except Exception as e:
            print(f"[WARNING] Could not apply theme: {e}")
    


def show_settings_dialog(parent=None):
    """
    Show settings dialog and return whether settings were saved.
    
    Returns:
        bool: True if user clicked Close (accepted), False if cancelled
    """
    dialog = SettingsDialog(parent)
    result = dialog.exec_()
    return result == QDialog.Accepted
