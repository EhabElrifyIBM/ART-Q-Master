...
class ExcelWorkbookAnalyzer:
    def __init__(self, root):
        self.root = root
        self.root.title("Archiver")
        self.root.geometry("900x700") # Increased height for footer
    ...
    def create_main_ui(self):
        ...
        # File selection
        ttk.Label(main_frame, text="Excel Workbook:", font=("Arial", 10, "bold")).grid(row=0, column=0, sticky=tk.W, pady=5)
        ...
        ttk.Button(main_frame, text="Analyze", command=self.analyze_workbook).grid(row=0, column=3, padx=5)
        
        # Status
        ttk.Label(main_frame, text="Status:", font=("Arial", 10, "bold")).grid(row=1, column=0, sticky=tk.W, pady=5)
        ...
        # Results display
        ttk.Label(main_frame, text="Analysis Results:", font=("Arial", 10, "bold")).grid(row=2, column=0, columnspan=4, sticky=tk.W, pady=(20, 5))
        
        self.results_text = scrolledtext.ScrolledText(main_frame, height=16, width=105, state=tk.DISABLED)
        self.results_text.grid(row=3, column=0, columnspan=4, sticky=(tk.W, tk.E, tk.N, tk.S), pady=5)
        
        # Action buttons
        button_frame = ttk.Frame(main_frame)
        button_frame.grid(row=4, column=0, columnspan=4, sticky=(tk.W, tk.E), pady=10)
        
        ttk.Button(button_frame, text="Export by Specific Month", command=self.export_by_month).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Export Older Than Days", command=self.export_old_cases).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Main Menu", command=self.return_to_main_menu).pack(side=tk.RIGHT, padx=5)
    ...
    def _create_progress_dialog(self, title="Progress"):
        ...
        log_text = tk.Text(dlg, height=18)
